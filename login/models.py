...
class Faculty(models.Model):
    ...
    faculty_initial = models.CharField(
...
#     approval = models.ForeignKey(Faculty, on_delete=models.CASCADE, null=True)
...
class Enrolled(models.Model):
    ...
    def __str__(self):
        return f"{self.student} enrolled in {self.course} (Approved: {self.is_approved})"
...
class Complain(models.Model):
    ...
    datetime = models.DateTimeField(
        default=datetime.now, blank=True, null=True)
    posted_by = models.ForeignKey(
        Student, on_delete=models.CASCADE, related_name="posted_by", blank=True, null=True)
    resolved_by = models.ForeignKey(
        Staff, on_delete=models.CASCADE, related_name="resolved_by", blank=True, null=True)
...
class Course_Content(models.Model):
    course_content_id = models.CharField(
        max_length=100, default=uuid.uuid4, unique=True)
    course_id = models.ForeignKey(Course, on_delete=models.CASCADE)
    course_content_tag = models.CharField(
        max_length=100, blank=True, null=True)
    course_content_description = models.CharField(
...
class Request(models.Model):
    ...
    def __str__(self):
        return f"Request by {self.student} for {self.course} to {self.faculty}"
