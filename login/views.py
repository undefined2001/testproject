...
from .models import Student, Staff, Faculty, Announcement, Complain, Course, Course_Content
    ...
    if request.method == 'POST':
        ...
        if password == password2:
            if User.objects.filter(email=email).exists():
                ...
                student = Student.objects.create( user=user, email=email, first_name = first_name, last_name= last_name)
                student.save()
                return redirect('studentLogin')
        else:
